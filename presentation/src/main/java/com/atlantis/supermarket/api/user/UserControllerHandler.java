...
public class UserControllerHandler {
    ...
    public ResponseEntity<ApiError> handleMyException(UserExistsException mex) {
	ApiValidationError apiValidation = new ApiValidationError("User", "El usuario " + mex.getUsername() + " ya existe");
	List<ApiSubError> list = new ArrayList<ApiSubError>();
	list.add(apiValidation);
	
	ApiError apiError = new ApiError(HttpStatus.CONFLICT, "El usuario ya existe", mex, list);
